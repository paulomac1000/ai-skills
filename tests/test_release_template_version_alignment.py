...
def test_mcp_consumer_templates_use_current_skill_version() -> None:
    ...
    conformance = yaml.safe_load((ROOT / "contracts/conformance-report.yaml.template").read_text(encoding="utf-8"))
    lock = yaml.safe_load((ROOT / "contracts/ai-skills.lock.yaml.template").read_text(encoding="utf-8"))
...
def test_generic_adoption_template_never_hardcodes_a_release_version() -> None:
    adoption = yaml.safe_load((ROOT / "contracts/adoption-assessment.yaml.template").read_text(encoding="utf-8"))
