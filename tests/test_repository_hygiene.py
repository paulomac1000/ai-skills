...
DOCUMENTED_EVIDENCE_FORMAT_VERSION = re.compile(
    r"\bevidence(?:\s+format)?\s+v[0-9]+\b",
    re.IGNORECASE,
)
...
def test_no_numbered_iteration_filenames_remain() -> None:
    offenders = [
        ...
        if "tests" not in path.relative_to(ROOT).parts and ITERATION_NAME.search(path.name)
    ]
    assert offenders == []
...
def test_current_evidence_format_has_no_numbered_public_name() -> None:
    ...
    for path in tracked_files():
        ...
        try:
            ...
            continue
        if DOCUMENTED_EVIDENCE_FORMAT_VERSION.search(text):
            offenders.append(path.relative_to(ROOT).as_posix())
    assert offenders == []
