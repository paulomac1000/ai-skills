...
_RISK_SEVERITY = {Risk.UNKNOWN: 0, Risk.READ: 1, Risk.WRITE: 2, Risk.SENSITIVE: 3, Risk.DESTRUCTIVE: 4, Risk.DANGEROUS: 5}
...
def infer_capability_profile(
    ...
    *,
    ...
    untrusted_risk, prefix = _untrusted_risk_signal(metadata.get("risk")), _prefixed_risk(name)
    ...
    if isinstance(annotations, Mapping):
        if annotations.get("destructiveHint") is True:
            ...
            if inferred is not previous:
                source = _append_source(source, "trusted-annotation" if trusted_server is True else "untrusted-annotation-escalation")
    ...
    explicit_sensitive = metadata.get("sensitive") is True or (trusted_policy is not None and trusted_policy.sensitive is True)
    ...
    requires_confirmation = metadata.get("requires_confirmation") is True or (trusted_policy is not None and trusted_policy.requires_confirmation is True)
    if metadata.get("idempotent") is False or (trusted_policy and trusted_policy.idempotent is False) or (trusted_contract and trusted_contract.idempotent is False):
        idempotent: bool | None = False
    elif (trusted_policy and trusted_policy.idempotent is True) or (trusted_contract and trusted_contract.idempotent is True):
        idempotent = True
    else:
        idempotent = None
...
def should_retry(*, error_code: str | None, attempt: int, operation_idempotent: bool, manifest: Mapping[str, Any] | None = None, response_retryable: bool | None = None, precondition_refreshed: bool = False) -> bool:
...
def _extract_protocol_error(response: Mapping[str, Any]) -> tuple[str, str]:
    ...
    if isinstance(structured, Mapping):
        code, message = structured.get("code"), structured.get("message") or structured.get("error")
    ...
    if isinstance(content, Sequence) and not isinstance(content, (str, bytes, bytearray)):
        messages = [item.get("text") for item in content if isinstance(item, Mapping) and isinstance(item.get("text"), str)]
        if messages:
...
def _failure(code: str, message: str, meta: Mapping[str, Any], correlation: str | None, retryable: bool | None = None) -> ResponseResult:
    return ResponseResult(False, meta=meta, error_code=code, error_message=message, retryable=retryable, correlation_id=correlation)
...
def handle_response(response: Mapping[str, Any]) -> ResponseResult:
    """Normalize only explicit recognized success shapes; empty/unknown payloads fail closed."""
    ...
    if response.get("isError") is True:
        ...
        return _failure(code, message, meta, correlation, response.get("retryable") if isinstance(response.get("retryable"), bool) else None)
    ...
    explicit_protocol_success = response.get("isError") is False and any(key in response for key in ("structuredContent", "content"))
    ...
    if not (explicit_legacy_success or explicit_protocol_success or recognized_payload_success):
        return _failure("MALFORMED_RESPONSE", "Tool response has no recognized success or error shape", meta, correlation)
...
def select_efficient_tool(tools: Iterable[Mapping[str, Any]], *, required_capabilities: Iterable[str], prefer_batch: bool = False) -> Mapping[str, Any] | None:
    ...
    for tool in tools:
        ...
        if required.issubset(declared):
            candidates.append((len(declared - required), 0 if bool(tool.get("batch")) == prefer_batch else 1, str(tool.get("name") or ""), tool))
    if not candidates:
        return None
...
def _schema_accepts_true(schema: Any) -> bool:
    ...
    return schema_type == "boolean" or (isinstance(schema_type, Sequence) and not isinstance(schema_type, (str, bytes, bytearray)) and "boolean" in schema_type)
...
def get_pagination_decision(meta: Mapping[str, Any], *, outcome_satisfied: bool, pages_seen: int, max_pages: int) -> PaginationDecision:
