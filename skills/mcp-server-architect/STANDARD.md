...
Every L2+ migration produces `migration-assessment.yaml` from `templates/migration-assessment.yaml.template`, covers every `mcp-server-architect` rule in `contracts/rule-catalog.yaml`, and follows `references/migration-assessment.md`. The assessment pins the immutable source revision, skill version, maturity target, profiles, scope, applicability matrix, implementation evidence, verification commands, preserved and intentionally changed behavior, removed legacy behavior, waivers, exact artifact identity, rollback, residual risks, and independent decision.
