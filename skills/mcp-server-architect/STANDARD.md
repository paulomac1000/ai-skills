...
Every L2+ design satisfies seven provider-neutral invariants and tests both a conforming and violating case. **RuntimeIdentity** exposes exact version, source revision, artifact identity, config revision, and instance generation; a current checkout never proves a stale or hand-patched deployment. **Diagnostic parity** exposes the same safe actionable failure class to the MCP caller that operators can see in logs. **Actionable preconditions** are reported before side effects and include all independently knowable violations. **Managed-resource ownership** blocks raw mutation before the provider call and names the authoritative reconciliation route. **Durable async progress** returns a durable operation identity plus observable status before request timeout can become ambiguous; client timeout is not terminal execution failure. **Bounded results** default to compact projections with explicit partial/truncation state and cursor or handle, while full detail is opt-in. **Scoped health** separates process, transport, auth, read, write, provider, and action-class readiness and binds conclusions to freshness/generation; read-good/write-bad is a valid degraded state, never whole-service green.

### Canonical control plane and projections

A control-plane server defines one canonical store/authority unless multi-master semantics are explicitly designed. External boards, indexes, or provider objects are projections/evidence, not writable truth by accident. Ingress records both observation context and affected canonical owner before deduplication or scheduling. Projection writes use a durable idempotent outbox with projection-generation identity, classified retry, bounded dead-letter handling, and authoritative reconciliation after ambiguous delivery. Raw provider adapters refuse mutation of canonically managed resources before side effects. Retarget, rename, transfer, split, or projection replacement preserves canonical entity identity/history through typed audited transitions. Work completed externally or by an operator has a supported evidence-backed completion transition rather than a fake lease or direct store edit. Human/agent projections expose a bounded actionable brief/status and reference larger evidence instead of presenting a thin or incomplete mirror as implementation-ready truth.

### Provider schema compatibility

Public tool schemas are validated recursively against a versioned provider profile rather than assuming protocol-valid JSON Schema is accepted by every function-calling provider. A profile records provider, contract revision, schema restrictions, and source evidence. Validation covers nested nullable array/object forms, type unions, `anyOf`/`oneOf`, `$ref`, defaults, `additionalProperties`, and placement of object `properties`/`required` and array `items`. Unknown or unsupported provider rules fail closed; they never activate optimistically. Release evidence exercises the exact candidate artifact through the official MCP client (`initialize`/`tools/list`) and binds the serialized public contract to artifact/source, client/protocol, and provider-profile revisions.

### Semantic façade profile

An agent-backed semantic façade keeps low-level upstream/provider tools internal by default and exposes stable domain intentions. The declared public budget is at most ten semantic operations and includes an explicit positive `max_public_schema_bytes`; CI records actual public tool count/schema bytes and fails when either declared budget is exceeded. Adding an internal provider does not add a public tool unless a new domain capability is intentional. Execution is deterministic discovery/probes/policy → structured evidence → optional bounded LLM classification/planning → deterministic authorization/execution → deterministic verification/read-back → bounded synthesis. The LLM cannot self-authorize mutation or prove measurable state. Long work uses durable jobs with compact status/result and delta continuation; privileged mutations use typed targets/actions/constraints and layered outcomes, not unrestricted `execute(prompt)`. Detailed evidence is persisted and retrieved by reference.
