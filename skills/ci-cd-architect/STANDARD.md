...
For provider-backed adoption, call the authority reusable workflow by a full SHA, prove the authority repository/SHA comes from the reusable-workflow identity rather than candidate input, compare the candidate trust declaration with that external authority, run provider-control preflight, correlate provider evidence to the exact candidate SHA, and require independent review. Classify incomplete migrations as `structurally-conformant`, `provider-preflight-blocked`, `provider-validation-pending`, or `independent-review-pending`; use `adopted` only after all provider-backed gates pass. Use `tools/classify_github_run_evidence.py` when a run result may be a zero-step/no-runner provider failure.
