...
def validate_audit_sequence(events: Sequence[Mapping[str, object]]) -> list[str]:
    ...
    for index, event in enumerate(events):
        ...
        if previous_effect is None:
            successful_effect_by_key[key_identity] = effect
        elif _success_effects_conflict(previous_effect, effect):
            findings.append(
                f"event[{index}]: CONFLICTING_TERMINAL_SUCCESS: {key_identity[2]} reports incompatible side effects"
            )
        else:
            findings.append(
                f"event[{index}]: DUPLICATE_TERMINAL_SUCCESS: {key_identity[2]} already has a canonical successful result"
            )
